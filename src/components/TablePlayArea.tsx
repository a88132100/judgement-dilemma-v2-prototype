...
import type { CardType, Faction, GameState, HumanFateDeclarationInput, HumanPlayInput, PlayerState, RoundPhase } from '../game/types';
...
export function TablePlayArea({ gameState, onGameStateChange }: TablePlayAreaProps) {
  ...
  const functionZoneClass = canChoosePlay && lockedFaction ? (lockedCard ? 'is-active-zone is-targeted-zone' : 'is-active-zone') : 'is-quiet-zone';
  ...
  function renderFunctionZone() {
    const content = lockedCard
      ? renderHiddenCardSlot('已伏置', cardImageByType[lockedCard], CARD_LABELS[lockedCard], { kind: 'function', cardType: lockedCard })
      : undefined;
    ...
    return renderStaticSlot('功能牌觸發區', '本回合未使用功能牌', content, functionZoneClass);
  }
            ...
            return (
              <article className="reveal-card-entry" key={player.id}>
                ...
                <div className="reveal-card-pair">
                  ...
                  <span className="reveal-mini-card">
                    <img src={player.playedCard ? cardImageByType[player.playedCard.type] : cardBackImage} alt="" onError={(event) => event.currentTarget.remove()} />
                    <small>{player.playedCard ? CARD_LABELS[player.playedCard.type] : '未使用'}</small>
                  </span>
                </div>
        ...
        {result ? (
          <div className="score-change-list">
            {result.situation.validPlayerIds.map((playerId) => {
              ...
              return (
                <details className="score-change-row" key={playerId}>
                  <summary>
                  ...
                  <dl>
                    <div>
                      <dt>基礎</dt>
                      <dd>{result.baseDeltaByPlayerId[playerId] ?? 0}</dd>
                    </div>
                    <div>
                      <dt>庇護</dt>
                      <dd>{result.shieldDeltaByPlayerId[playerId] ?? 0}</dd>
                    </div>
                    <div>
                      <dt>反擊</dt>
                      <dd>{result.counterDeltaByPlayerId[playerId] ?? 0}</dd>
                    </div>
                    <div>
                      <dt>宿命</dt>
                      <dd>{result.fateDeltaByPlayerId[playerId] ?? 0}</dd>
                    </div>
                    <div>
                      <dt>承諾</dt>
                      <dd>{result.commitmentDeltaByPlayerId[playerId] ?? 0}</dd>
                    </div>
                  </dl>
                </details>
              );
